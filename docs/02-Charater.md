...
### TIL | 2026-07-22 | HeroAnimInstance 추가

- 이제 Base -> Character -> Hero의 AnimInstance를 추가
- Character는 플레이어를 포함한 모든 캐릭터가 움직여야 하기에 속도에 따른 애니메이션만 지정
- HeroCharacter는 본인만의 독자적인 애니메이션을 가져야 하기에 이번 강의에서 추가됨. (Relax 상태 추가)

### 핵심

- 애니메이션 블루프린트에서 Idle과 이어지는 또 하나의 상태 추가
- 해당 상태는 조건에 맞을 때 상태 전이 가능
- Hero만의 독자적 애니메이션 추가

### 느낀 점

- 역시 이전에 많이 삽질해봤던 부분이라 진행이 빨랐고 재밌었음
- 움직임 유무 조건에 대한 코드 작성과 애니메이션 추가가 이렇게 간단할 줄 몰랐음
- 기존의 방식이었다면, 애님 인스턴스 클래스가 아닌 캐릭터 컨트롤러나, 인풋 컴포넌트에서 입력이 있는지 없는지 확인했을텐데 그럴 필요 없이 애님 인스턴스에서 현재 움직이고 있는지에 대한 정보만 받아서 애니메이션 설정이 가능하다는 것, 그리고 그 조건들을 쉽게 블루프린트에서 설정할 수 있다는 것에 대해 많이 느낌
- 초반 아키텍처의 중요성을 다시 느끼는 중

![Character](./images/RelaxState.png)
